...
namespace PhysicsEngine
{
	...
	public:
		...
		{ 
			...
			GetShape(0)->setLocalPose(PxTransform(PxVec3(.0f, .0f, .0f), PxQuat(45.f, PxVec3(.0f, 1.f, .0f))));
		}
	};
	...
	{
	public:
		Compound(const PxTransform& pose = PxTransform(PxIdentity), PxVec3 dimensions = PxVec3(.5f, .5f, .5f), PxReal density = 1.f)
			: DynamicActor(pose)
		{
			CreateShape(PxBoxGeometry(dimensions), density);
			GetShape(0)->setLocalPose(PxTransform(PxVec3(1.f, .0f, .0f)));
			CreateShape(PxBoxGeometry(dimensions), density);
			GetShape(1)->setLocalPose(PxTransform(PxVec3(0.f, .0f, .0f)));
		}
	};
