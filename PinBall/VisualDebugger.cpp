...
namespace VisualDebugger
{
	...
	void Init(const char *window_name, int width, int height)
	{
		...
		camera = new Camera(PxVec3(0.0f, 60.0f, 60.0f), PxVec3(0.f,-1.0f,-0.8f), 5.f);
	...
	void HUDInit()
	{
		...
		//Handles score hud
		score = to_string(scene->score);
		lives = to_string(scene->lives);
		hud.AddLine(EMPTY, "Score: " + score);
		hud.AddLine(EMPTY, "Lives: " + lives);
	...
	void Paddles(int key)
	{
		string actor = scene->GetSelectedActor()->getName();
		switch (toupper(key))
		{
			//Left Paddle
		case '4':
			cout << scene->GetSelectedActor()->getName() << endl;
			if (actor == "Paddle1")
			{
				scene->GetSelectedActor()->addForce(PxVec3(0, 0, -1)* gForceStrength * 300);
			}
			else
			{
		...
		case '6':
			...
			//Launch Ball
		case '8':
			if (scene->launchReady == true)
			{
				if (actor == "PinBall")
				{
					scene->GetSelectedActor()->addForce(PxVec3(0, 0, -1)* gForceStrength * 50);
					cout << "LETS LAUNCH" << endl;
				}
				else
				{
					scene->SelectNextActor();
